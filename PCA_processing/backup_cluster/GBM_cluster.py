...
def read_data():
    wb = xl.open_workbook("./../external_database/TCGA/6. TCGA-GBM/sample.xls")
    ws = wb.sheet_by_index(0)

    directory = './../external_database/TCGA/6. TCGA-GBM/'
...
if __name__ == "__main__":
    ...
    np.savetxt('./../PCA_tcga/GBM/PC_dat.dat', projection.T, fmt='%f')
    np.savetxt('./../PCA_tcga/GBM/eigenvectors_dat.dat', eigenvectors.T, fmt='%f')
    np.savetxt('./../PCA_tcga/GBM/eigenvalues_normalized_dat.dat', eigenvalues_normalized, fmt='%f')
    np.savetxt('./../PCA_tcga/GBM/eigenvalues_dat.dat', eigenvalues, fmt='%f')
    np.savetxt('./../PCA_tcga/GBM/eigenvectorsT_dat.dat', eigenvectors, fmt='%f')

    principal = eigenvectors[:, eigenvalues.argmax()]
    index = np.argpartition(-np.abs(principal), 20)[:20]
    components = principal[index]

    np.savetxt('./../PCA_tcga/GBM/20_index_dat.dat', index, fmt='%i')
    np.savetxt('./../PCA_tcga/GBM/20_component_dat.dat', components, fmt='%f')
    np.savetxt('./../PCA_tcga/GBM/normal_dat.dat', normal, fmt='%i')
    np.savetxt('./../PCA_tcga/GBM/tumor_dat.dat', tumor, fmt='%i')